    ...
    if seq==1
        ...
        for k=1:3 %%三相
            for t=ii:simulation_step:ii+0.02-simulation_step
                E_predict=0;
                zhuanhua=round(t/50e-6)-prd*step_one_cycle;
                for j=1:fre_l
...
exit_time=1.18; %退出机制启用时间
exit_prd=5; %5个周期大于最小熄弧角就退出
...
step=round(ii/simulation_step);
 ...
 if seq<=66
            ...
            if Pulse_five(1,1)<=0.5&&Pulse_five(2,1)>=0.5 %%找打触发开始时刻
                t_beta=seq+1;%%统一到一个周期  阀电流过零比触发脉冲过零晚一个步长
                E_a=A(prd*400+1:(prd+1)*400,2);
                E_b=A(prd*400+1:(prd+1)*400,3);
                E_c=A(prd*400+1:(prd+1)*400,4);
                E_bc=E_b-E_c;
...
%                 etime(t22,t11)
                ...
                gama_save1(6*(prd-49)-5,1)=gama_without_har;
                ...
                %程序退出机制
                if ii>exit_time&&min(gama_real(step-exit_prd*step_one_cycle:step-1,1))>=7.2*pi/180
            ...
            %% 6号阀
                ...
                if ii>=t1&&ii<=t2
                    ...
                    load gama_save
                    gama_save1=gama_save;
                    gama_save1(6*(prd-49)-4,1)=gama_without_har;
                    gama_save=gama_save1;
                    save('gama_save.mat','gama_save')
                ...
                if ii>=t1&&ii<=t2
                        ...
                        load gama_save
                        gama_save1=gama_save;
                        gama_save1(6*(prd-49)-3,1)=gama_without_har;
                        gama_save=gama_save1;
                        save('gama_save.mat','gama_save')
                ...
                if ii>=t1&&ii<=t2
                            ...
                            load gama_save
                            gama_save1=gama_save;
                            gama_save1(6*(prd-49)-2,1)=gama_without_har;
                            gama_save=gama_save1;
                            save('gama_save.mat','gama_save')
                ...
                if ii>=t1&&ii<=t2
                                ...
                                load gama_save
                                gama_save1=gama_save;
                                gama_save1(6*(prd-49)-1,1)=gama_without_har;
                                gama_save=gama_save1;
                                save('gama_save.mat','gama_save')
                            ...
                            if Pulse_four(1,1)<=0.5&&Pulse_four(2,1)>=0.5 %%找打触发开始时刻
                                t_beta=seq+1;%%统一到一个周期  阀电流过零比触发脉冲过零晚一个步长
                E_a=A(prd*400+1:(prd+1.5)*400,2); %%最后一个周期取1.5周期，电压过零点在下一个周期
                E_b=A(prd*400+1:(prd+1.5)*400,3);
                E_c=A(prd*400+1:(prd+1.5)*400,4);
                                E_ac=E_a-E_c;
                ...
                if ii>=t1&&ii<=t2
                                ...
                                load gama_save
                                gama_save1=gama_save;
                                gama_save1(6*(prd-49)-0,1)=gama_without_har;
                                gama_save=gama_save1;
                                save('gama_save.mat','gama_save')
